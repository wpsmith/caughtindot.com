...
abstract class Libsyn implements Iterator, Countable, ArrayAccess  {
	...
	protected $text_dom = LIBSYN_TEXT_DOMAIN;
	protected $plugin_name = "Libsyn/Wordpress";
	protected $plugin_version = "1.2.2.3";
	protected $api_table_name = "libsyn_podcast_plugin";
	protected $api_base_uri = "https://api.libsyn.com";
	protected $minimum_php_version = 5.4;
	protected $recommended_php_version = 5.6;
	protected $max_php_version = 7.4;
	...
	public function getVars() {
		...
		//Add Utilities
		$this->utilities = new \Libsyn\Utilities();

		//Add Site Details
		$this->site_details = (function_exists('get_blog_details')) ? new \Libsyn\Site(get_blog_details()) : null;

		//Set WP Current User
		$this->current_user_id = (function_exists('get_current_user_id')) ? get_current_user_id() : null;
	}
     ...
     * @return <Libsyn\Api>
     */
	public function retrieveApiById( $user_id , $disable_errors=false) {

		$api_table_name = $this->getApiTableName();
		$data = get_user_option($api_table_name, $user_id);
		$isJson = $this->utilities->isJson($data);
		if($isJson) $data = json_decode($data);
