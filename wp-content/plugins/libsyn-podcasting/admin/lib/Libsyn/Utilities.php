...
class Utilities extends \Libsyn {
	...
	public static function logPluginData() {
		...
		if ( intval($urlParams['libsyn_debuginfo']) === 1 ) {
			...
			if ( ! function_exists( 'get_plugins' ) ) {
				...
				} elseif ( file_exists(ABSPATH . WPINC . '/plugin.php') ) {
					require_once ABSPATH . WPINC . '/plugin.php';
				} elseif ( file_exists(realpath( __DIR__ . '/../../..') . '/wp-admin/includes/plugin.php') ) {
	 ...
	 *
	 * @since 1.2.2.2
	 * @param  int $post_id Current WP Post Id
	 * @return mixed         Embed Url | false on error
	 */
	public static function getEmbedUrlGeneric( $post_id ) {
		if ( empty($post_id) ) return false;
		$item_id = get_post_meta($post_id, 'libsyn-item-id', true);
		...
		// https://html5-player.libsyn.com/embed/episode/id/10049438/height/90/theme/custom/thumbnail/yes/direction/forward/render-playlist/no/custom-color/bfd66e/
	}
