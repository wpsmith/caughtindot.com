...
class Block extends \Libsyn\Post {
	...
	public static function editorAssets() {
		...
		wp_enqueue_script(
			...
			plugins_url( LIBSYN_DIR . '/lib/libsyn-podcasting-gutenberg/dist/blocks.build.js' ), // Block.build.js: We register the block here. Built with Webpack.
		...
		wp_enqueue_style(
			...
			plugins_url( LIBSYN_DIR . '/lib/libsyn-podcasting-gutenberg/dist/blocks.editor.build.css' ), // Block editor CSS.
	...
	public static function shortcodeCallback( $attributes ) {
		...
		$atts['download_link_text'] = ( !empty($downloadLinkTextMeta) ) ? $downloadLinkTextMeta : 'top';
